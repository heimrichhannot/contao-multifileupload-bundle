var MultiFileUpload,
    ...
    __getField = function(dropzone, name) {
        var fields = dropzone.element.querySelectorAll('input[name="' +
            (typeof name !== 'undefined' ? name + '_' : '') + dropzone.options.paramName + '"]');
    ...
    __defaults = {
        init: function() {
            ...
            this.on('thumbnail', function(file, dataUrl) {
                ...
                if (typeof response.result === 'undefined') {
                    dropzone.emit('error', file,
                        dropzone.options.dictResponseError.replace('{{statusCode}}', ': Empty response'),
                        response);
                    return;
                }
                ...
                function handleResponse(file, response) {
                    ...
                    if (response.filenameOrigEncoded === rawurlencode(file.name) && response.uuid !==
                        'undefined') {
            ...
            // add mock files
            var initialFiles = __getField(this, 'formattedInitial'),
                dropzone = this;
